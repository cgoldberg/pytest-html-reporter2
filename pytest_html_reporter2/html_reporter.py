...
class HTMLReporter:
    ...
    def pytest_runtest_teardown(self, item, nextitem):
        ...
        if (self.rerun is not None) and (max_rerun() is not None):
            self.previous_test_name(ConfigVars._test_name)
    ...
    def pytest_sessionfinish(self, session):
        if ConfigVars._suite_name is not None:
            self.append_suite_metrics_row(ConfigVars._suite_name)
    ...
    def archive_data(self, base, filename):
        ...
        if os.path.isfile(path) is True:
            ...
            if isfile(join(base, f)):
                ...
                os.rename(
                    base + "/" + f,
                    os.path.join(base + "/archive", fname[0] + "_" + str(ConfigVars._start_execution_time) + fname[1]),
                )
    ...
    def append_test_metrics_row(self):
        ...
        floating_error_text = FloatingError(
            full_msg=str(ConfigVars._current_error), runt=str(time.time()).replace(".", "")
        )

        if (self.rerun is not None) and (max_rerun() is not None):
            if (ConfigVars._test_status == "FAIL") or (ConfigVars._test_status == "ERROR"):
            ...
            if (ConfigVars._pvalue == max_rerun() + 1) or (ConfigVars._test_status == "PASS"):
                if ((ConfigVars._test_status == "FAIL") or (ConfigVars._test_status == "ERROR")) and (
                    ConfigVars.screen_base != ""
                ):
                    self.generate_screenshot_data()
                ...
                (ConfigVars._test_status == "xFAIL")
                or (ConfigVars._test_status == "xPASS")
                or (ConfigVars._test_status == "SKIP")
            ):
        ...
        self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name), {}).setdefault(
            "tests", {}
        ).setdefault(len(ConfigVars._scenario) - 1, {})["status"] = str(ConfigVars._test_status)
        self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name), {}).setdefault(
            "tests", {}
        ).setdefault(len(ConfigVars._scenario) - 1, {})["message"] = str(ConfigVars._current_error)
        self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name), {}).setdefault(
            "tests", {}
        ).setdefault(len(ConfigVars._scenario) - 1, {})["test_name"] = str(ConfigVars._test_name)

        if (self.rerun is not None) and (max_rerun() is not None):
            self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name), {}).setdefault(
                "tests", {}
            ).setdefault(len(ConfigVars._scenario) - 1, {})["rerun"] = str(self.rerun)
        else:
            self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name), {}).setdefault(
                "tests", {}
            ).setdefault(len(ConfigVars._scenario) - 1, {})["rerun"] = "0"
    ...
    def generate_screenshot_data(self):
        ...
        if len(ConfigVars._test_name) >= 19:
            ConfigVars._screenshot_test_name = ConfigVars._test_name[-17:]
        _screenshot_error = ConfigVars._current_error

        ConfigVars.screen_img.save(ConfigVars.screen_base + "/pytest_screenshots/" + str(_screenshot_name) + ".png")
    ...
    def append_suite_metrics_row(self, name):
        ...
        for i in self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name) - 1, {})["tests"]:
            if (
                "ERROR"
                in self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name) - 1, {})["tests"][i][
                    "status"
                ]
            ):
                ...
                "FAIL"
                == self.json_data["content"]["suites"].setdefault(len(ConfigVars._test_suite_name) - 1, {})["tests"][i][
                    "status"
                ]
            ):
    ...
    def _test_names(self, name, **kwargs):
        if (self.rerun is None) or (max_rerun() is None):
            ConfigVars._scenario.append(name)
        try:
            if kwargs["clear"] == "yes":
                ConfigVars._scenario = []
        except Exception:
            pass
    ...
    def renew_template_text(self, logo_url):
        ...
        # template_text = template_text.replace("__generated_date__", str(datetime.datetime.now().strftime("%b %d %Y, %H:%M"))) # noqa
    ...
    def generate_json_data(self, base):
        ...
        for i in suite:
            ...
            try:
                if self.json_data["status"] == "FAIL":
                    break
            except KeyError:
                if len(ConfigVars._test_suite_name) == i + 1:
                    self.json_data["status"] = "PASS"
        ...
        for i, val in enumerate(f):
            with open(val) as json_file:
                ...
                archive_row_text = ArchiveRow(
                    astate=state(data["status"].lower())[0], astate_color=state(data["status"].lower())[1]
                )
                if value == "current":
                ...
                atime = "".join(
                    list(filter(lambda x: ":" in x, time.ctime(float(data["start_time"])).split(" ")))
                ).rsplit(":", 1)[0]
    ...
    def update_trends(self, base):
        ...
        with open(f2[0]) as json_file:
            ...
            ConfigVars.trends_label.append(
                str(time_converter(atime)).upper()
                + " | "
                + str(adate.date().strftime("%b"))
                + " "
    ...
    def attach_screenshots(self, screen_name, test_suite, test_case, test_error):
        ...
        _screenshot_details = ScreenshotDetails(
            screen_name=str(screen_name), ts=str(test_suite), tc=str(test_case), te=str(test_error)
        )
